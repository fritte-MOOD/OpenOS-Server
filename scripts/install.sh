...
read -rp "Enter the disk to install to (e.g. sda, nvme0n1): " DISK
DISK_PATH="/dev/$DISK"
...
# Layout:
#   1: ESP   (1 GB)   — EFI boot
#   2: Root  (32 GB)  — NixOS system
#   3: Data  (rest)   — /data (persistent, survives reinstalls)
...
if git clone "$REPO_URL" "$OPENOS_DIR/flake" 2>&1; then
  ...
  # Check if we're running from the OpenOS installer ISO
  ...
  elif [ -d "/run/media/"*"/openos-flake" ]; then
...
echo ""
read -rp "Press Enter to reboot (or Ctrl+C to stay in the live environment)..."
reboot
